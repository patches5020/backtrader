...
def calculate_extension(
    ...
    For "down", raw targets (swing_high - diff*ratio) can go negative or to
    zero whenever the swing range exceeds roughly 38% of swing_high -- a
    routine occurrence for volatile, lower-priced assets over a normal
    lookback window. A negative or zero price is never valid, so each level
    is floored at a small positive fraction of swing_low (1%) rather than
    allowed through -- this preserves the descending TP1>TP2>TP3>TP4 order
    while guaranteeing every returned price is usable (e.g. by a live order,
    which would otherwise be rejected outright by the exchange).
    """
    if swing_high <= swing_low:
        raise ValueError("swing_high must be greater than swing_low")

    diff = swing_high - swing_low
    base_floor = swing_low * 0.01  # never return <= 0, or a price that isn't usable
    levels: dict[str, float] = {}

    for idx, ratio in enumerate(EXTENSION_RATIOS):
        if direction == "up":
            levels[f"{ratio}"] = swing_low + diff * ratio
        elif direction == "down":
            # per-level floor decreases slightly with idx so TP1>TP2>TP3>TP4
            # stays strictly true even when every raw target is floored
            level_floor = base_floor * (1 - 0.1 * idx)
            levels[f"{ratio}"] = max(swing_high - diff * ratio, level_floor)
        else:
