...
def test_extension_never_returns_non_positive_prices():
    # regression test: a wide swing range relative to price (typical for
    # volatile lower-priced assets like XRP) used to produce negative TP levels
    levels = calculate_extension(swing_high=1.31, swing_low=0.20, direction="down")
    values = list(levels.values())
    assert all(v > 0 for v in values)
    assert values[0] > values[1] > values[2] > values[3]  # still strictly descending
